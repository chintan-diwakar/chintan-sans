...
Do not link `css/flamex.css` on a website. That file lists every static weight and is for the specimen, not for production.

## Files to copy

```
css/flamex-web.css
fonts/web/FlamexSans-latin.woff2
fonts/web/FlamexSans-latin-ext.woff2
fonts/web/FlamexSans-Italic-latin.woff2
fonts/web/FlamexSans-Italic-latin-ext.woff2
```
...
```html
<link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/chintan-diwakar/flamex-sans@v1.200/css/flamex-web.css">
```
...
| Page text | Download |
| --- | --- |
| English, or other Latin text | `FlamexSans-latin.woff2`, 98 KB |
| Italic on that page | plus `FlamexSans-Italic-latin.woff2`, 108 KB |
...
Greek and Cyrillic are not in the light build. For those, use `fonts/variable/FlamexSans-Variable.woff2` (about 344 KB) instead of the Latin subset.
...
- `font-family` lists that name both "Flamex Sans" and "Flamex Sans Variable". The light CSS uses one family name.
